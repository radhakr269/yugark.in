    ...
    <>
      ...
      <main className="pt-32 pb-24 bg-[#050505] bg-perspective-grid bg-aurora-glow">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 sm:space-y-16">
          <div className="grid grid-cols-1 min-[600px]:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
            {filteredStudies.map((study, idx) => (
              <motion.div
                ...
                className="relative rounded-xl sm:rounded-2xl bg-[#0B0B0B]/85 backdrop-blur-xl border border-[#D4B06A]/20 overflow-hidden hover:border-[#D4B06A]/60 transition-all duration-300 group flex flex-col justify-between shadow-[0_20px_40px_rgba(0,0,0,0.8)] hover:-translate-y-1.5"
              >
                ...
                <div>
                  ...
                  <div className="px-3 sm:px-4 py-1.5 sm:py-2.5 bg-[#141414] border-b border-white/5 flex items-center justify-between">
                    ...
                    <div className="px-2 sm:px-3 py-0.5 rounded-md bg-[#080808] border border-white/5 text-[9px] sm:text-[10px] text-neutral-400 font-mono tracking-wider truncate max-w-[150px] sm:max-w-none">
                      {study.client.toLowerCase().replace(/\s+/g, '')}.yugark.studio
                    </div>
                    <Globe className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-neutral-500" />
                  </div>
                  ...
                  <div className="relative h-40 sm:h-60 overflow-hidden">
                    ...
                    <div className="absolute top-2 left-2 sm:top-3 sm:left-3 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-black/85 backdrop-blur-md text-[9px] sm:text-[10px] uppercase font-bold tracking-wider text-[#F0D28F] border border-[#D4B06A]/40 shadow-lg">
                      {study.industry}
                    </div>
                  </div>
                  ...
                  <div className="p-4 sm:p-7 space-y-3 sm:space-y-4">
                    <h3 className="font-serif text-lg sm:text-2xl font-semibold text-white group-hover:text-[#F0D28F] transition-colors leading-snug">
                      {study.title}
                    </h3>
                    <p className="text-xs text-neutral-300 leading-relaxed font-sans font-light line-clamp-3 sm:line-clamp-none">
                      {study.summary}
                    </p>

                    {/* Results Stat Chips */}
                    <div className="grid grid-cols-3 gap-1.5 sm:gap-2 pt-3 sm:pt-5 border-t border-neutral-800/80">
                      {study.results.map((res, rIdx) => (
                        <div key={rIdx} className="text-center p-1.5 sm:p-2 rounded-lg bg-[#141414]/60 border border-white/5">
                          <span className="block font-serif text-sm sm:text-lg font-bold text-[#F0D28F]">
                            {res.value}
                          </span>
                          <span className="text-[8px] sm:text-[9px] text-neutral-400 uppercase tracking-tight block truncate">
                            {res.label}
                          </span>
                        </div>
                      ))}
                ...
                <div className="p-4 sm:p-7 pt-0">
                  <button
                    ...
                    className="w-full py-2.5 sm:py-3.5 rounded-xl bg-[#141414] border border-white/10 text-neutral-200 group-hover:text-black group-hover:bg-gradient-to-r group-hover:from-[#D4B06A] group-hover:to-[#C9A35E] font-semibold text-[11px] sm:text-xs uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer shadow-md"
                  >
